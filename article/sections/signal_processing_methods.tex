...
The section contains the following categories: time-domain and frequency-domain methods (Sections~\ref{subsec:sp_time}--\ref{subsec:sp_freq}) operate directly on the measured signal; time-frequency, decomposition, and signal enhancement methods (Section~\ref{subsec:sp_tf}) address non-stationarity and improve the signal before feature extraction; physics-based and model-driven methods (Section~\ref{subsec:sp_model}) incorporate lubrication physics to convert processed signals into physically interpretable quantities; and machine learning and data fusion methods (Sections~\ref{subsec:sp_ml}--\ref{subsec:feature_fusion}) learn mappings from processed features to lubrication condition labels or regression targets. Figure~\ref{fig:sp_taxonomy} provides an overview of this taxonomy.

\begin{figure}[H]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  root/.style={circle, draw=black!85, fill=black!85, text=white,
               align=center, font=\sffamily\bfseries, text width=2.4cm, inner sep=4pt,
               preaction={fill=black!15, transform canvas={xshift=0.8pt, yshift=-1pt}}},
  cat/.style={rectangle, rounded corners=9pt, draw=none, fill=black!9,
              align=center, font=\sffamily\bfseries\small,
              text width=2.6cm, inner sep=5pt, minimum height=1.2cm},
  meth/.style={rectangle, rounded corners=5pt, draw=black!20, line width=0.5pt,
               fill=white, align=center, font=\sffamily\footnotesize,
               text width=2.4cm, inner sep=3pt, minimum height=0.9cm,
               preaction={fill=black!15, transform canvas={xshift=0.8pt, yshift=-1pt}}},
  arr/.style={line width=0.9pt, black!55},
  execute at begin node={\hyphenpenalty=10000\relax},
]
\def\rcat{3.6}   % radius of the category ring
\def\rmeth{6.6}  % radius of the method ring

% Root
\node[root] (R) at (0,0) {Signal Processing Methods for LCM};

% Categories (first level), clockwise from the left
\foreach \name/\ang/\txt in {
    T/180/{Time-Domain\\Statistics},
    F/120/{Frequency\\Domain},
    TF/60/{Time-Frequency,\\Decomposition\\\& Enhancement},
    P/0/{Physics-Based \&\\Model-Driven},
    ML/-60/{Machine Learning\\\& Data-Driven},
    FF/-120/{Feature\\Fusion}}
{
  \node[cat] (\name) at (\ang:\rcat) {\txt};
  \draw[arr] (R) -- (\name);
}

% Methods (second level): outside their category, offset by \t cm along the ring
\foreach \parent/\ang/\t/\txt in {
    T/180/-1.7/{Moments \& energy metrics\\(RMS, kurtosis,\\Hjorth, PCR)},
    T/180/0/{Speed \& load\\normalisation},
    T/180/1.5/{Threshold\\methods},
    F/120/-1.35/{Spectral statistics \&\\shape descriptors},
    F/120/1.35/{Multi-band\\hit rate \& WPI},
    TF/60/-2.1/{STFT \&\\kurtogram},
    TF/60/0/{EMD, VMD\\\& SGMD},
    TF/60/2.1/{Minimum entropy\\deconvolution},
    P/0/-1.5/{Spring model\\(ultrasonic)},
    P/0/0/{Electrical circuit\\models},
    P/0/1.5/{$\kappa$\,/\,$\Lambda$ as\\regression targets},
    ML/-60/-1.35/{Classical ML\\(LASSO, shallow NN)},
    ML/-60/1.35/{Deep learning\\(CNN, GNN, CycleGAN)},
    FF/-120/0/{Graph-based\\multi-sensor fusion}}
{
  \node[meth, shift={(\ang-90:\t)}] (m) at (\ang:\rmeth) {\txt};
  \draw[arr] (\parent) -- (m);
}

\end{tikzpicture}%
}
\caption{Taxonomy of signal processing method families applied to lubrication condition monitoring of rolling element bearings. The six method categories (inner ring) correspond to Sections~\ref{subsec:sp_time}--\ref{subsec:feature_fusion}, and the methods reviewed within each category are arranged on the outer ring. Methods are organised by processing family rather than by sensing modality; the master cross-reference matrix (Table~\ref{tab:sp_crossref}) maps each family to the sensing modalities for which LCM application has been demonstrated in the reviewed literature.}
...
Table~\ref{tab:sp_crossref} summarises the successfull application of SP method families to each sensing modality, as found in the reviewed literature. A filled cell indicates the combination has been demonstrated for LCM in at least one reviewed study.
...
>{\raggedright\arraybackslash}p{3.4cm}
 ...
 >{\centering\arraybackslash}X
 >{\centering\arraybackslash}X }
& \multicolumn{5}{c}{\textbf{Passive}} & \multicolumn{5}{c}{\textbf{Active}} \\
\textbf{SP Method Family}
& \rotatebox{75}{\textbf{Vibration}}
& \rotatebox{75}{\textbf{\shortstack[l]{Acoustic\\Emission}}}
& \rotatebox{75}{\textbf{\shortstack[l]{Ultrasound\\(passive)}}}
& \rotatebox{75}{\textbf{\shortstack[l]{Sound /\\Microphone}}}
& \rotatebox{75}{\textbf{Thermal}}
& \rotatebox{75}{\textbf{\shortstack[l]{Active\\Ultrasonic}}}
& \rotatebox{75}{\textbf{Electrical}}
& \rotatebox{75}{\textbf{\shortstack[l]{Magnetic\\/ Oil}}}
& \rotatebox{75}{\textbf{Optical}}
& \rotatebox{75}{\textbf{SAW}}
\\
\hline
...
\hline
Spectral shape  & \checkmark & \checkmark &            &            &            &            &            &            &            &            \\
\hline
Multi-band hit-rate / WPI   &            & \checkmark &            &            &            &            &            &            &            &            \\
\hline
STFT          & \checkmark &            &            &            &            &            &            &            &            &            \\
\hline
Signal decomposition &            &            &            &            &            &            &            & \checkmark &            &            \\
\hline
Deconvolution         & \checkmark &            &            &            &            &            &            &            &            &            \\
\hline
