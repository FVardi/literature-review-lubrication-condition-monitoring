...
\hline
% TODO: cross-check "Vibration $\leq$50kHz" and "Sound/microphone $\leq$50kHz" sampling-rate figures against the accelerometer/microphone bandwidth reconciliation in sensing_methods_and_empirical_studies.tex (ADI Table 3: MEMS accelerometer DC--20kHz+, piezo accelerometer 2.5--30kHz+, MEMS microphone ~20kHz, MEMS ultrasonic ~100kHz) and Table~\ref{tab:signal_origins} row 2 (physical_origins_of_signals.tex) once those are finalised, so sampling-rate ceilings here stay consistent with whatever bandwidth figures are settled on there.
